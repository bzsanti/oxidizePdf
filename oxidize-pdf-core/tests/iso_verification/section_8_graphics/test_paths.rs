...
iso_test!(
    ...
    "Basic path construction operators (m, l, c, h)",
    {
        ...
        let notes = if passed {
            "Successfully generated PDF with path construction"
        } else {
            "Failed to generate PDF with paths"
        };
...
iso_test!(
    ...
    "Path painting operators (S, f, B)",
    {
        ...
        let level_achieved = if passed { 2 } else { 1 };
        let notes = if passed {
            "Successfully generated PDF with path painting operations"
        } else {
            "Failed to generate PDF with path painting"
        };
