...
iso_test!(
    ...
    "Graphics state stack operations (q/Q operators)",
    {
        ...
        let notes = if passed {
            "Successfully generated PDF with graphics state operations"
        } else {
            "Failed to generate PDF with graphics state"
        };
...
iso_test!(
    ...
    "Line width setting (w operator)",
    {
        ...
        let level_achieved = if passed { 2 } else { 1 };
        let notes = if passed {
            "Successfully generated PDF with line graphics"
        } else {
            "Failed to generate PDF with line operations"
        };
