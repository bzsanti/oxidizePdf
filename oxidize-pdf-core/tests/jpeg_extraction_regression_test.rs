...
fn test_jpeg_extraction_no_byte_duplication() {
    ...
    let analysis = analyzer.analyze_page(0).expect("Failed to analyze page 0");
    ...
    assert!(
        analysis.image_ratio > 0.9,
        "Expected high image ratio for scanned page"
    );
    ...
    if std::path::Path::new(extracted_jpeg_path).exists() {
        ...
        println!(
            "✅ JPEG extraction test passed - file size: {} bytes",
            file_size
        );
    } else {
        panic!(
            "Expected JPEG file was not created at {}",
            extracted_jpeg_path
        );
    }
}
