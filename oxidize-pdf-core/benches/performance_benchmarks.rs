...
use oxidize_pdf::performance::{
    ...
    PerformancePage, ResourceKey, ResourcePool, StreamingOptions, StreamingPageContent,
    StreamingPdfWriter,
};
...
fn bench_parallel_generation(c: &mut Criterion) {
    ...
    for num_pages in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(
            ...
            num_pages,
            |b, &num_pages| {
                let options = ParallelGenerationOptions::memory_efficient().with_max_threads(1); // Force sequential
                let generator = ParallelPageGenerator::new(options).unwrap();
                ...
                b.iter(|| {
                    ...
                    for i in 0..*num_pages {
                        let content = StreamingPageContent {
                            width: 595.0,
                            height: 842.0,
                            content_streams: vec![ContentStream::from_text(
                                &format!("Page {}", i),
                                100.0,
                                200.0,
                                "F1",
...
fn bench_intelligent_compression(c: &mut Criterion) {
    ...
    let test_cases = [
        ...
        (
            "png_image",
            generate_test_image(5120),
            ContentType::ImagePng,
        ),
        (
            "jpeg_image",
            generate_jpeg_data(5120),
