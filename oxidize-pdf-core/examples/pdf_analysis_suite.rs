...
impl PDFAnalyzer {
    ...
    pub fn analyze_directory(
        ...
        if self.verbose {
            println!(
                "🎉 Directory analysis completed: {} files processed",
                reports.len()
            );
        }
    ...
    fn generate_recommendations(&self, report: &mut PDFAnalysisReport) {
        ...
        if report.content_analysis.image_analysis.total_image_size > 10_000_000 {
            report
                .recommendations
                .push("Consider optimizing images to reduce file size".to_string());
        }

        if report.content_analysis.font_analysis.system_fonts > 0 {
            report
                .recommendations
                .push("Consider embedding fonts for better portability".to_string());
        }

        if !report.security_analysis.is_encrypted && report.file_info.file_size > 1_000_000 {
            report
                .recommendations
                .push("Consider adding password protection for sensitive documents".to_string());
        }
            ...
            > report.content_analysis.page_count / 2
        {
            report
                .recommendations
                .push("Many pages contain no text - consider OCR if scanned document".to_string());
        }
    }
    ...
    fn detect_issues(&self, report: &mut PDFAnalysisReport) {
            ...
            .is_empty()
        {
            report.warnings.push(format!(
                "Text encoding issues detected on {} pages",
                report
                    .content_analysis
                    .text_analysis
                    .text_encoding_issues
...
fn create_sample_analysis_pdf() -> Result<PathBuf> {
    ...
    page1
        .graphics()
        .show_text_at("PDF Analysis Suite Demo Document", 100.0, 750.0, 16.0)?;
    ...
    page2
        .graphics()
        .show_text_at("Page 2: Mixed Content", 100.0, 750.0, 14.0)?;
    page2.graphics().show_text_at(
        "This page demonstrates various PDF features:",
        100.0,
        700.0,
        12.0,
    )?;
    ...
    page3
        .graphics()
        .show_text_at("Page 3: Minimal Content", 100.0, 750.0, 12.0)?;
...
fn print_analysis_report(report: &PDFAnalysisReport) {
    ...
    println!(
        "📊 File Size: {:.2} MB",
        report.file_info.file_size as f64 / 1_000_000.0
    );
    ...
    println!(
        "   • Pages with text: {}/{}",
        text.pages_with_text, report.content_analysis.page_count
    );
    ...
    println!(
        "   • Image size: {:.2} MB",
        images.total_image_size as f64 / 1_000_000.0
    );
    ...
    println!(
        "   • Analysis time: {:?}",
        report.performance_metrics.total_analysis_time
    );
    println!(
        "   • Pages/second: {:.2}",
        report.performance_metrics.pages_per_second
...
fn main() -> Result<()> {
    ...
    if results_dir.exists() {
        ...
        println!(
            "📁 Analyzed {} PDF files in directory",
            directory_reports.len()
        );

        // Summary statistics
        let total_pages: usize = directory_reports
            .iter()
            .map(|r| r.content_analysis.page_count)
            .sum();
        let total_size: u64 = directory_reports
            .iter()
