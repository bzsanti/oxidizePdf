...
/// Decode a Windows-1252 (PDF WinAnsi) byte to its Unicode character.
///
/// Exact inverse of [`winansi_encode_char`] over the defined code points. The
/// `0x00..=0x7F` and `0xA0..=0xFF` ranges decode identity (ASCII / Latin-1);
/// the `0x80..=0x9F` window carries the Windows-1252 specials. The five
/// unassigned code points (0x81, 0x8D, 0x8F, 0x90, 0x9D) decode to the
/// same-valued C1 control character (matching `byte as char`); they carry no
/// glyph in WinAnsi-encoded fonts.
pub fn winansi_decode_char(byte: u8) -> char {
    match byte {
        0x80 => '\u{20AC}', // €
        0x82 => '\u{201A}', // ‚
        0x83 => '\u{0192}', // ƒ
...
mod tests {
    ...
    #[test]
    fn winansi_decode_char_is_inverse_of_encode() {
        // For every byte that maps to a defined WinAnsi glyph, decode then encode
        // must round-trip back to the same byte.
        for byte in 0u8..=255 {
            let ch = winansi_decode_char(byte);
            if let Some(reencoded) = winansi_encode_char(ch) {
                assert_eq!(reencoded, byte, "round-trip failed for byte {byte:#04x}");
            }
        }
    }

    #[test]
    fn winansi_decode_char_specific_points() {
        assert_eq!(winansi_decode_char(0x80), '€');
        assert_eq!(winansi_decode_char(0x97), '—'); // emdash
        assert_eq!(winansi_decode_char(0x92), '\u{2019}'); // right single quote
        assert_eq!(winansi_decode_char(0xED), 'í'); // Latin-1 identity
        assert_eq!(winansi_decode_char(b'A'), 'A'); // ASCII identity
                                                    // Unassigned Windows-1252 code points decode to their C1 control char.
        assert_eq!(winansi_decode_char(0x81), '\u{0081}');
    }
