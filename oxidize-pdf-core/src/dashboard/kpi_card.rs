...
impl KpiCard {
    ...
    pub fn with_trend_period<T: Into<String>>(mut self, change: f64, direction: TrendDirection, period: T) -> Self {
        self.trend = Some(TrendInfo {
            change,
            direction,
            period: period.into(),
...
impl KpiCard {
    ...
    fn calculate_layout(&self, card_area: ComponentPosition) -> KpiCardLayout {
        ...
        let title_area = ComponentPosition::new(
            card_area.x + padding,
            current_y,
            card_area.width - padding * 2.0 - (if self.icon.is_some() { icon_size + padding } else { 0.0 }),
            line_height,
        );
    ...
    fn render_sparkline(
        ...
        let max_val = sparkline.data.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
        ...
        let graphics = page.graphics();
        graphics.set_stroke_color(colors.primary)
                .set_line_width(1.5);
    ...
    fn format_value(&self) -> String {
        match &self.value_format {
            ...
            ValueFormat::Currency { symbol, decimal_places } => {
                if let Ok(num) = self.value.parse::<f64>() {
                    format!("{symbol}{num:.prec$}", symbol = symbol, num = num, prec = *decimal_places as usize)
                } else {
                    self.value.clone()
                }
            }
            ...
            ValueFormat::Number { decimal_places, thousands_separator } => {
                if let Ok(num) = self.value.parse::<f64>() {
...
mod tests {
    ...
    fn test_kpi_card_formatting() {
        let card = KpiCard::new("Price", "1299.99")
            .as_currency("$");
