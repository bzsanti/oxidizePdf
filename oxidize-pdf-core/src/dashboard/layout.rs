...
impl DashboardLayout {
    ...
    fn calculate_column_utilization(&self, components: &[Box<dyn DashboardComponent>]) -> f64 {
        ...
        let total_span: u32 = components.iter()
            .map(|c| c.get_span().columns as u32)
            .sum();
    ...
    pub fn layout_components(
        ...
        while row_start < components.len() {
            ...
            if current_y - row_height < start_y - total_height {
                tracing::warn!("Dashboard components exceed available height, stopping at row {}", positions.len() / row_components.len());
                break;
            }
...
impl LayoutManager {
    ...
    fn apply_mobile_layout(&self, components: &mut [Box<dyn DashboardComponent>]) -> Result<(), PdfError> {
    ...
    fn apply_tablet_layout(&self, components: &mut [Box<dyn DashboardComponent>]) -> Result<(), PdfError> {
        for component in components.iter_mut() {
