...
iso_test!(
    ...
    "Annotation dictionary structure and required entries verification",
    {
        ...
        page.text()
            ...
            .at(55.0, 580.0)
            .write("Annotation infrastructure ready - supports /Type /Annot with /Subtype entries")?;
        ...
        let has_rect_coords_1 = pdf_string.contains("100") && pdf_string.contains("650") 
                                && pdf_string.contains("150") && pdf_string.contains("25");
        let has_rect_coords_2 = pdf_string.contains("300") && pdf_string.contains("120") && pdf_string.contains("30");
        ...
        let has_coord_refs = pdf_string.contains("[100, 650, 250, 675]") || 
                            pdf_string.contains("100") && pdf_string.contains("250") && pdf_string.contains("675");
        let has_rect_notation = pdf_string.contains("/Rect") || pdf_string.contains("llx lly urx ury");
        
        // Verify annotation infrastructure text
        let has_annot_text = pdf_string.contains("/Type") && pdf_string.contains("/Subtype") && pdf_string.contains("/Rect");
        ...
        let has_precise_coords = pdf_string.contains("105") && pdf_string.contains("305") && pdf_string.contains("665");
        ...
        let passed = has_coordinate_arrays && has_annot_text && has_infrastructure_text
                    && has_annotation_areas && has_precise_coords && has_coord_system_text
                    && has_valid_structure && has_fonts && sufficient_content;
...
iso_test!(
    ...
    "Annotation coordinate system and rectangle boundary validation",
    {
        ...
        let all_grid_points = has_grid_100_700 && has_grid_200_700 && has_grid_300_700 && has_grid_400_700;
        
        // Verify annotation rectangle coordinates
        let has_rect1_coords = pdf_string.contains("80") && pdf_string.contains("650") && pdf_string.contains("60") && pdf_string.contains("30");
        let has_rect2_coords = pdf_string.contains("160") && pdf_string.contains("80") && pdf_string.contains("35");
        let has_rect3_coords = pdf_string.contains("260") && pdf_string.contains("70") && pdf_string.contains("25");
        let has_rect4_coords = pdf_string.contains("350") && pdf_string.contains("90") && pdf_string.contains("40");
        let all_rect_coords = has_rect1_coords && has_rect2_coords && has_rect3_coords && has_rect4_coords;
        ...
        let has_axes = pdf_string.contains("50") && pdf_string.contains("500") && pdf_string.contains("550") && pdf_string.contains("750");
        ...
        let passed = all_grid_points && all_rect_coords && has_coord_explanation 
                    && has_rect_format && has_axes && has_axis_labels && has_sufficient_geometry
                    && has_valid_structure && has_fonts && sufficient_content;
        let level_achieved = if passed { 3 } else { 2 };
        let notes = if passed {
...
iso_test!(
    ...
    "Annotation appearance infrastructure and visual representation",
    {
        ...
        page.text()
            ...
            .at(50.0, 560.0)
            .write("/N=Normal, /R=Rollover, /D=Down appearance states for interactive annotations")?;
        ...
        let has_state_doc = pdf_string.contains("Normal") && pdf_string.contains("Rollover") && pdf_string.contains("Down");
        let has_infrastructure_doc = pdf_string.contains("color spaces") && pdf_string.contains("graphic states");
        ...
        let passed = has_highlight_color && has_note_fill && has_note_border && has_stamp_borders 
                    && has_stamp_text && has_freetext_bg && has_freetext_content && has_ap_doc 
                    && has_state_doc && has_infrastructure_doc && has_rich_appearance
                    && has_valid_structure && uses_colors && has_fonts && sufficient_content;
        let level_achieved = if passed { 3 } else { 2 };
        let notes = if passed {
...
iso_test!(
    ...
    "Annotation integration with page content and resources",
    {
        ...
        page.text()
            .set_font(Font::TimesRoman, 12.0)
            .at(72.0, 720.0)
            .write("This document demonstrates how annotations integrate with existing page content.")?;
        ...
        let has_base_content = pdf_string.contains("Key technical term") && pdf_string.contains("Important document section");
        ...
        let has_margin_note = pdf_string.contains("Margin Note") && pdf_string.contains("Additional info");
        ...
        let integration_documented = has_zorder_doc && has_coordinate_doc && has_resource_doc && has_annots_doc;
        ...
        let complex_integration = fill_operations >= 3 && stroke_operations >= 5 && move_operations >= 4 && line_operations >= 4;
        ...
        let passed = text_highlight_overlap && margin_integration && stamp_integration 
                    && integration_documented && complex_integration && has_valid_structure 
                    && has_fonts && uses_colors && sufficient_content;
        let level_achieved = if passed { 3 } else { 2 };
        let notes = if passed {
...
mod tests {
    ...
    fn test_annotations_structure_infrastructure() -> PdfResult<()> {
        ...
        let pdf_string = String::from_utf8_lossy(&pdf_bytes);
        let has_coordinates = pdf_string.contains("100") && pdf_string.contains("650") 
                             && pdf_string.contains("150") && pdf_string.contains("25");
        let has_highlight = pdf_string.contains("300") && pdf_string.contains("120");
        let has_annotation_text = pdf_string.contains("Annotation Area") && pdf_string.contains("Highlight Style");

        println!("✓ Annotation components - coordinates: {}, highlight: {}, text: {}", 
        ...
        assert!(pdf_bytes.len() > 1600, "PDF should have annotation infrastructure content");
        assert!(has_coordinates, "PDF should have precise annotation coordinates");
        assert!(has_highlight, "PDF should have highlight-style annotation");
        assert!(has_annotation_text, "PDF should have annotation documentation");
