...
impl Operation {
    ...
    pub fn category(&self) -> OperationCategory {
        match self {
            ...
            Operation::PageProcessing | Operation::ImageProcessing | Operation::FontProcessing => OperationCategory::Processing,
            Operation::ContentCompression | Operation::ResourceDeduplication => OperationCategory::Optimization,
            Operation::MemoryAllocation | Operation::ParallelProcessing => OperationCategory::System,
...
impl PerformanceMonitor {
    ...
    pub fn start_operation(&self, operation: Operation) -> OperationToken {
        ...
        *next_token += 1;
        ...
        self.active_operations.lock().unwrap().insert(token.0, active_op);
        token
    }
    ...
    pub fn end_operation(&self, token: OperationToken) -> Duration {
        ...
        if let Some(active_op) = active_op {
            ...
            self.completed_operations.write().unwrap().push(completed_op.clone());

            // Update operation statistics
            let mut stats = self.operation_stats.write().unwrap();
            let op_stats = stats.entry(active_op.operation).or_insert_with(|| OperationStats::new(active_op.operation));
    ...
    pub fn get_stats(&self) -> PerformanceMetrics {
        ...
        for (operation, stats) in &operation_stats {
            ...
            let cat_stats = category_stats.entry(category).or_insert_with(CategoryStats::default);
    ...
    pub fn get_operation_stats(&self, operation: Operation) -> Option<OperationStats> {
        self.operation_stats.read().unwrap().get(&operation).cloned()
    }
...
impl DurationHistogram {
    fn new() -> Self {
        let buckets = vec![
            DurationBucket { upper_bound: Duration::from_micros(100), count: 0 },      // 0.1ms
            DurationBucket { upper_bound: Duration::from_micros(500), count: 0 },      // 0.5ms
            DurationBucket { upper_bound: Duration::from_millis(1), count: 0 },        // 1ms
            DurationBucket { upper_bound: Duration::from_millis(5), count: 0 },        // 5ms
            DurationBucket { upper_bound: Duration::from_millis(10), count: 0 },       // 10ms
            DurationBucket { upper_bound: Duration::from_millis(50), count: 0 },       // 50ms
            DurationBucket { upper_bound: Duration::from_millis(100), count: 0 },      // 100ms
            DurationBucket { upper_bound: Duration::from_millis(500), count: 0 },      // 500ms
            DurationBucket { upper_bound: Duration::from_secs(1), count: 0 },          // 1s
            DurationBucket { upper_bound: Duration::from_secs(5), count: 0 },          // 5s
            DurationBucket { upper_bound: Duration::MAX, count: 0 },                   // >5s
        ];
...
impl PerformanceMetrics {
    ...
    pub fn summary(&self) -> String {
        let slowest = self.slowest_operation()
            .map(|(op, duration)| format!("{} ({:.2}ms)", op.name(), duration.as_secs_f64() * 1000.0))
            .unwrap_or_else(|| "None".to_string());

        let most_frequent = self.most_frequent_operation()
...
mod tests {
    ...
    fn test_statistics_collection() {
        ...
        let page_stats = monitor.get_operation_stats(Operation::PageProcessing).unwrap();
    ...
    fn test_operation_categories() {
        ...
        assert_eq!(Operation::ImageProcessing.category(), OperationCategory::Processing);
        assert_eq!(Operation::ContentCompression.category(), OperationCategory::Optimization);
    ...
    fn test_performance_metrics_analysis() {
        ...
        let _ = monitor.time_operation(Operation::PdfGeneration, || sleep(Duration::from_millis(2)));
        let _ = monitor.time_operation(Operation::ImageProcessing, || sleep(Duration::from_millis(1)));
        let _ = monitor.time_operation(Operation::PdfGeneration, || sleep(Duration::from_millis(3)));
