...
use oxidize_pdf::graphics::{
    Color, GraphicsContext, IccProfile, IccProfileManager, IndexedColorManager, IndexedColorSpace,
    StandardIccProfile,
};
...
fn create_icc_profiles_page(doc: &mut Document) -> Result<()> {
    ...
    if let Some(profile) = icc_manager.get_profile(&cmyk_name) {
        ...
        graphics.text_at(
            Point::new(50.0, y - 20.0),
            &format!(
                "Components: {}, ISO standard for printing",
                profile.components
            ),
        );
    }
...
fn create_indexed_colors_page(doc: &mut Document) -> Result<()> {
    ...
    graphics.text_at(
        ...
        &format!(
            "Total colors: {}, Max index: {}",
            web_safe.color_count(),
            web_safe.max_index()
        ),
    );
    ...
    graphics.text_at(
        ...
        &format!(
            "Total grays: {}, Max index: {}",
            grayscale.color_count(),
            grayscale.max_index()
        ),
    );
