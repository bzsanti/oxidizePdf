...
use oxidize_pdf::forms::{
    calculation_system::{
        CalculationSettings, FieldFormat, FormCalculationSystem, JavaScriptCalculation,
        NegativeStyle, PercentMode, SeparatorStyle, SimpleOperation,
    },
    calculations::{
        ArithmeticExpression, Calculation, CalculationEngine, CalculationFunction, ExpressionToken,
        FieldValue, Operator,
    },
    BorderStyle, FieldType, TextField, Widget, WidgetAppearance,
};
...
fn create_invoice_page(doc: &mut Document) -> Result<(), PdfError> {
    ...
    // Add title
    page.add_text(
        "Invoice with Automatic Calculations",
        50.0,
        700.0,
        Font::HelveticaBold,
        16.0,
    ...
    page.add_text(
        "Note: All totals are automatically calculated",
        50.0,
        150.0,
        Font::Helvetica,
        8.0,
...
fn create_percentage_calculator_page(doc: &mut Document) -> Result<(), PdfError> {
    ...
    // Instructions
    page.add_text(
        "Enter values to see automatic percentage calculations:",
        50.0,
        650.0,
        Font::Helvetica,
        12.0,
...
fn create_loan_calculator_page(doc: &mut Document) -> Result<(), PdfError> {
    ...
    let monthly_payment = principal * (monthly_rate * (1.0 + monthly_rate).powf(num_payments))
        / ((1.0 + monthly_rate).powf(num_payments) - 1.0);

    page.add_text("Monthly Payment:", 70.0, 460.0, Font::Helvetica, 12.0)?;
    page.add_text(
        &format!("${:.2}", monthly_payment),
        200.0,
        460.0,
        Font::HelveticaBold,
        12.0,
...
fn create_grade_calculator_page(doc: &mut Document) -> Result<(), PdfError> {
    ...
    // Instructions
    page.add_text(
        "Enter scores to calculate weighted grade:",
        50.0,
        650.0,
        Font::Helvetica,
        12.0,
...
fn demonstrate_calculation_system() -> Result<(), PdfError> {
    ...
    println!(
        "  Tax (20%): ${:.2}",
        engine.get_field_value("tax").unwrap().to_number()
    );
    println!(
        "  Net Pay: ${:.2}",
        engine.get_field_value("net_pay").unwrap().to_number()
    );
...
mod tests {
    ...
    fn test_invoice_calculation() {
        ...
        calc_system
            .set_field_value("qty", FieldValue::Number(10.0))
            .unwrap();
        calc_system
            .set_field_value("price", FieldValue::Number(5.50))
            .unwrap();
    ...
    fn test_percentage_discount() {
        ...
        calc_system
            .set_field_value("original", FieldValue::Number(100.0))
            .unwrap();
        calc_system
            .set_field_value("discount", FieldValue::Number(20.0))
            .unwrap();
    ...
    fn test_grade_average() {
        ...
        calc_system
            .set_field_value("g1", FieldValue::Number(90.0))
            .unwrap();
        calc_system
            .set_field_value("g2", FieldValue::Number(85.0))
            .unwrap();
        calc_system
            .set_field_value("g3", FieldValue::Number(95.0))
            .unwrap();
