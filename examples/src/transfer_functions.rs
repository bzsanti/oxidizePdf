...
fn main() -> Result<()> {
    ...
    let gamma_state = ExtGState::new()
        .with_gamma_correction(2.2);  // Standard monitor gamma
    
    // Example 2: Linear transfer function
    // Maps colors linearly with custom slope and intercept
    let linear_state = ExtGState::new()
        .with_linear_transfer(0.8, 0.1);  // Reduces contrast slightly
    
    // Example 3: Identity transfer (no change)
    let identity_state = ExtGState::new()
    ...
    let cmyk_state = ExtGState::new()
        .with_transfer_function(cmyk_functions);
